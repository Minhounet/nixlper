...
### Added
- **Fuzzy picker for navigate** (`functions_navigation.sh`): `CTRL+X+N` now opens an `fzf` incremental picker after the directory listing when `fzf` is installed — folders first, then files, each prefixed with a `[D]`/`[F]` marker and a 1-based index. Type digits to jump to that numbered entry, or letters to fuzzy-filter by name. Selecting a folder `cd`s into it and refreshes the listing; selecting a file opens it with `$NIXLPER_EDITOR`. The numbered shortcuts (`v1`, `cdf1`, `n1`, etc.) remain active regardless of fuzzy mode. Configurable via `nconf`: `NIXLPER_NAVIGATE_FUZZY` (bool, default `true`).
