...
- **Port/service quick-check** (`functions_processes.sh`): `pc PORT` resolves a port to the process name and PID listening on it, and prints a suggested action (`ik --port` or `kill -9`) without prompting to kill. Reuses the `ss`/`netstat` detection already used by `ik --port`.
