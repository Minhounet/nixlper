...
### Added
- **Fuzzy search for recent directories** (`functions_recent_dirs.sh`): `rd` / `CTRL+X+J` now opens an `fzf` incremental fuzzy filter when `fzf` is installed — type a few characters to narrow the list live, arrows to move, Enter to jump, Esc to cancel (IntelliJ-style "recent files"). Falls back to the existing numbered picker when `fzf` isn't installed. Configurable via `nconf`: `NIXLPER_RECENT_DIRS_FUZZY` (bool, default `true`).
