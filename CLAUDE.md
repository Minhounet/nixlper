...
**What the script does — pure bash, zero external dependencies, no LLM calls, no cost to run
beyond GitHub Actions minutes:**
...
4. Updates the README badge (version number) and its "What's new" one-liner — built by joining
   the promoted bullets' bold lead-ins (`build_summary` in the script), not generated by a
   model. Verified working correctly in the first real dry run (see the "Scheduled release"
   workflow run history).

An earlier version of this script optionally called the Claude API for a nicer one-sentence
summary. It was removed: this project has no separate Anthropic API billing (only Claude Code
and claude.ai), so that path could never actually run here, and keeping unreachable code around
just invited confusion about whether the automation depends on a paid API. It doesn't, for
anything — the whole pipeline is deterministic bash from end to end.
...
which would mean `create_release_on_tag.yml` never fires. No other secret or credential is
needed — the script never calls an external API.
