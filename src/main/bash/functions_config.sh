...
_NIXLPER_CONFIG_VARS=(
  ...
  "NIXLPER_BOOKMARKS_FUZZY|bool|true|Use fzf incremental filter for bookmark picker (bd) when fzf is installed|common"
  ...
  "NIXLPER_RECENT_DIRS_FUZZY|bool|true|Use fzf incremental fuzzy filter for recent dirs (rd) when fzf is installed|common"
  "NIXLPER_LAST_COMMAND_MAX|int|50|Maximum number of history entries considered by last_command (lc)|advanced"
  "NIXLPER_LAST_COMMAND_FUZZY|bool|true|Use fzf incremental fuzzy filter for last command (lc) when fzf is installed|common"
  "NIXLPER_NAVIGATE_FUZZY|bool|true|Use fzf incremental fuzzy filter for navigate (CTRL+X+N) when fzf is installed|common"
  "NIXLPER_KILL_FUZZY|bool|true|Use fzf unified picker for interactive kill (ik) when fzf is installed|common"
...
function _nconf_create_user_conf() {
  ...
  mkdir -p "${HOME}/.config/nixlper"
  {
    ...
    printf "# export NIXLPER_RECENT_DIRS_FUZZY=true\n"
    printf "# export NIXLPER_BOOKMARKS_FUZZY=true\n"
    printf "# export NIXLPER_LAST_COMMAND_FUZZY=true\n"
    printf "# export NIXLPER_NAVIGATE_FUZZY=true\n"
    printf "# export NIXLPER_KILL_FUZZY=true\n\n"
    ...
    printf "# export NIXLPER_LAST_COMMAND_MAX=50\n"
